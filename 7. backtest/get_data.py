...
def get_data():
    ticker = ['btcusdt', 'ethusdt', 'trxusdt', 'shibusdt', 'solusdt', 'dotusdt',
       ...
       'dydxusdt', 'dashusdt', '1inchusdt', 'xmrusdt', 'compusdt', 'bnbusdt',
       'iotausdt', 'sklusdt', 'paxusdt', 'wbtcusdt']
    data_all=dict()
    count=1

    for i in ticker:
        end=time.mktime(time.strptime("2022-03-01 00:00:00","%Y-%m-%d %H:%M:%S"))
        start_time = int(end)- 365 * 86400
        end_time=int(end)
        period = '5min'
        target = i
        data = Huobi_kline(symbol=target, period=period, start_time=start_time,end_time=end_time, get_full_market_data=True, col_with_asset_name=False)
        data_all[i]=data
